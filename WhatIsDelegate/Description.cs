...
namespace WhatIsDelegate
{
    ...
    {
        // void 리턴타입과 매개변수를 받지 않는 함수를 저장할 수 있는데이터 타입을 선언
        ...
        public void DelegateDesc()
        {
             ...
             * 
             * 대리자(위임/델리게이트)
             * 대리자는 delegate 키워드를 사용하여 만든다. 대리자는 함수 자체를 데이터 하나로 보고 의미 그대로 다른 메서드를
             * 대신 실행하는 기능이다.
             * 
             * 한 번에 하나 이상을 대ㅣㄴ해서 호출할 수 있는 개념이다.
             * 
             *  - 자동차 개체를 예로 들면, 대리운전처럼 대리자가 집까지 좌회전(), 우회전(), 직진(), 주차() 등 동작을 대신해서 할 수 있게하는 
             *    개념과 비슷하다.
