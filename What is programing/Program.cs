...
namespace WhatIsProgramming
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            /**
             * 컴퓨터는 하드웨어(Hardware)와 소프트웨어(Sogteware)로 구성된다. 하드웨어는 PC, 스마트퐌과 같은
             ...
             * 기계어는 인간에게 상당히 불편한(난해한) 언어이기 때문에 사람이 이해하기 쉬운 프로그래밍 언어가 만들어지게 된다.
             ...
             * ildasm
             * 
             * C#은 .Net(닷넷)을 위한 많은 언어중 하나로, 마이크로소프트의 닷넷 플랫폼을 기반으로 한다.
             ...
             * C#은 컴파일 기반 언어이다.
             * C#은 강력한 형식(Strongly type)의 언어이다.
             ...
             * 
             * 네임스페이스 : 자주 사용하는 네임스페이스를 위족에 미리 선언해 둘 수 있다.
            ...
            /**
             * 이스케이프 스퀸스
             * C#은 WriteLine() 메서드에서 사용할 확장 문자를 제공하는데,
             * 이를 이스케이프 시퀸스(Escapesequence)라고 한다.
             * 역슬래쉬(\) 기호와 특정 문자를 조합하면 특별한 기능을 사용 할 수 있다.
             * 어떠한 이스케이프 시퀸스가 있는지 검색하면 알 수 있다.(https://learn.microsoft.com/en-us/cpp/c-language/escape-sequences?view=msvc-170)
             */
            ...
            /**
             * 문자열 보관법
             * 문자열 보관법(String interpolation) 또는 문자열 템플릿(Template)이라고도 한다.
            ...
            Console.WriteLine("정회태\n(게임콘텐츠제작) 모바일 게임 개발자 양성과정C \n(전공)게임 프로그래밍 기초 기술");
            ...
            //int가 4byte 인 이유 64비트 기반 운영체제에서 ~

            /**
             * 변수 사용할 때 주의사항 (규칙)
             * 변수의이름을 지을 때는 다음 규칙을 지켜야 한다.
             * 변수의 첫 글자는 반드시 문자로 지정한다. 숫자는 변수의 이름의 첫 글자로 사용할 수 없다.
             * 길이는 255자 이하로 하고 공백을 표현할 수 없다.
             * 영문자와 숫자, 언더스코어(_) 조합으로 사용하며, 기타 특수 기호는 사용할 수 없다.
             * C#에서 사용하는 키워드는 사용할 수 없다.
             * 변수는 대, 소문자를 구분하고, 일반적으로 소문자로 시작한다.
             * 변수는 타인이 보더라도 이해할 수 있는 이름으로 시작한다.
             */

            /**
             * 데이터 형식
             * 변수에 저장할 수 있는 데이터의 종류를 자료형(Data type) 이라고 한다.
             * int, string, bool, double, object 등 C#에서 기본으로 제공하는 데이터 형식을 기본 형식(Primitive type)이라고 한다.
             * 
             ...
             * object : C#에서 모든 자료의 부모형 데이터 타입 (모든 데이터를 저장 가능)
             */
             ...
             * 
             * 상수
             * 변수를 선언할 때 앞에 const 키워드를 붙이면 상수(constant)가 된다. 한 번 상수로 선언된 변수는 다시 값을 바꿀 수 없고,
             * 반드시 선언과 동시에 초기화해야 한다. 이러한 const 키워드를 붙인 변수를 상수 또는 지역(Local) 상수라고 한다. 상수는 주로 대문자로 표현한다.
             * 
             * 리터럴
             ...
             * 숫자 형식을 표현할 때 언더스코어(_) 문자는 무시한다. 이를 이용하면 긴 숫자를 표시할 때 가독성을 높일 수 있다.
             */
            ...
            const int CONST_FIVE = 5;       //CONST_FIVE = 상수형 리터럴, 5= 정수형 리터럴
            ...
            int radius;
            radius = 5;
            Console.WriteLine(radius * radius * PI); // 반지름을 정수로 설정, 파이를 상수로 설정한 후 원의 넓이 출력
             ...
             * 이러한 null 가능 형식에는 아무런 값도 없음을 의미하는 null 을 대입할 수 있다.
             */

            int? nullNumber = null;     // <- nullable 형식
             ...
             * 자동 타입 추론이랑 컴파일러가 대입하는 값 또는 변수의 데이터 타입으로 다른 한 쪽의 데이터 타입을 추론하는 기능을 의미한다.
             * 
             * default 값
             * C# 6.0 버전부터는 자동 타입 추론의로 기본 형식에 default 값을 대입할 수 있다.
            ...
            var autovariableDouble = 3.14;

            /**
             * 열거형 형식
             * C#에서 열거형(Enumeration) 형식은 기억하기 어려운 상수들을 기억하기 쉬운 이름 하나로 묶어 관리하는 표현 방식이다.
             * 일반적으로 열거형으로 줄여 말한다. 열거형은 enum 키워드를 사용하고 있고 이늄 또는 이넘으로 읽는다.
            ...
            /**
             * 입출력에 대해서
             * 프로그램을 입력할 때마다 서로 다른 값을 입력받으려면 콘솔에서 입력한 값을 변수에 저장할 수 있어야 한다.
            ...
            /**
             * 형식변환
             * Console.Readline() 메서드를 사용하여 콘솔에서 입력받은 데이터는 문자열이다. 문자열 대신 정수나 실수 데이터를 입력받고 싶다면
        ...
        enum Align { TOP, BOTTOM, LEFT, RIGHT, RANDOM, VALUE };     //메서드 범위 안에서 정의 불가
